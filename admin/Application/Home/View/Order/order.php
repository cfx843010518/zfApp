...
<title>查看订单</title>
<link rel="stylesheet" type="text/css" href="__PUBLIC__/css/themes/default/easyui.css">
<link rel="stylesheet" type="text/css" href="__PUBLIC__/css/themes/icon.css">
...
if($page_id>=1 && $page_num>1){
	echo "<a href=findOrder?page_id=1&from=$from&to=$to&status=$status>第一页</a>";
}
if($page_id>1 && $page_num>1){
	echo "<a href=findOrder?from=$from&to=$to&status=$status&page_id=".($page_id-1).">上一页</a>";
}
if($page_id>=1 && $page_num>$page_id){
	echo "<a href=findOrder?from=".$from."&to=".$to."&status=".$status."&page_id=".($page_id+1).">下一页</a>";
}
if($page_id>=1 && $page_num>1){
	echo "<a href=findOrder?from=$from&to=$to&status=$status&page_id=".$page_num.">尾页</a>";
}
?>
</td>
</tr>
