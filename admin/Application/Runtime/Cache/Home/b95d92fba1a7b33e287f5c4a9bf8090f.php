...
<script type="text/javascript">
	$(document).ready(function(){
		$('#order_date_year').change(function(){
			...
			window.location.href='orderGatherDetail?year='+order_date_year+'&month='+order_date_month;
		});
	...
	function expOrderDetailAll(){
		var order_date_year = $('#order_date_year').val();
		var order_date_month = $('#order_date_month').val();
		window.location.href = "expOrderDetailAll?order_date_year="+order_date_year+"&order_date_month="+order_date_month;
	}
</script>
<body>
<div id="header">
...
<center>
<div id="right">
	<p style="background:#628e37; padding-left:10px; color:#FFF;">您当前的位置：订单管理－＞订单年/月销量明细汇总</p>
	...
	<a href="toGo?url=Report/index">返回首管理页</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:void(0);" onclick="expOrderDetailAll()">导出为Excel表</a>
	<?php
 session_start(); if(!isset($_SESSION['user'])){ echo "<script>alert('请先登录');</script>"; echo "<script>location.href='/zfApp/admin/index.php';</script>"; }else{ ?>
	...
	<td bgcolor="#FFFFFF"><div>购买用户</div></td>
	<td bgcolor="#FFFFFF"><div>支付方式</div></td>
	<td bgcolor="#FFFFFF"><div>支付时间</div></td>
	<td bgcolor="#FFFFFF"><div>送货方式</div></td>
 ...
 foreach($rs as $key => $value){ ?>
	<tr title="message">
		...
		<td bgcolor="#FFFFFF" style="text-align:center;"><?php echo $value['user_account'];?></td>
		<td bgcolor="#FFFFFF" style="text-align:center;"><?php echo $value['pay_way'];?></td>
		<td bgcolor="#FFFFFF" style="text-align:center;"><?php echo $value['pay_date'];?></td>
		<td bgcolor="#FFFFFF" style="text-align:center;"><?php echo $value['delivery_way'];?></td>
	...
	<?php }}?>
	<tr>
		<td bgcolor="#FFFFFF" colspan="7">总计</td>
