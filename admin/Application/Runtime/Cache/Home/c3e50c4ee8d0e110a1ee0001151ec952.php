...
<?php  session_start(); if(!isset($_SESSION['user'])){ echo "<script>alert('请先登录');</script>"; echo "<script>location.href='/zfApp/admin/index.php';</script>"; }else{ if($hs==0){ echo "暂无供应商"; }else{ ?>
...
<?php
echo "本站共有"; echo $hs; echo "条记录&nbsp;"; echo "每页显示"; echo $size; echo "条&nbsp;"; echo "第"; echo $page_id; echo "页/共"; echo $page_num; echo "页&nbsp;"; if($page_id>=1 && $page_num>1){ echo "<a href=showAllSupplier?page_id=1&searchText=$searchText>第一页</a>"; } if($page_id>1 && $page_num>1){ echo "<a href=showAllSupplier?searchText=$searchText&page_id=".($page_id-1).">上一页</a>"; } if($page_id>=1 && $page_num>$page_id){ echo "<a href=showAllSupplier?searchText=$searchText&page_id=".($page_id+1).">下一页</a>"; } if($page_id>=1 && $page_num>1){ echo "<a href=showAllSupplier?searchText=$searchText&page_id=".$page_num.">尾页</a>"; } ?>
</td>
</tr>
</table>
<?php }?> 
