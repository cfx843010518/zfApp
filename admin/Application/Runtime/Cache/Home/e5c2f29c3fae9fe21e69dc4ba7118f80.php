...
<div id="right" >
	...
	<form action="/zfApp/admin/index.php/Product/saveProduct" method="post" onsubmit="return sjyz(this)" enctype="multipart/form-data">
	...
	<?php
 session_start(); if(!isset($_SESSION['user'])){ echo "<script>alert('请先登录');</script>"; echo "<script>location.href='/zfApp/admin/index.php';</script>"; }else{ for($i=0;$i<count($types);$i++){ echo "<option value=".$types[$i]['pro_type_id'].">".$types[$i]['pro_type_name']."</option>"; } } ?>
	</select>
	</td>
	</tr>
	<tr>
	...
	<?php
 for($i=0;$i<count($suppliers);$i++){ echo "<option value=".$suppliers[$i]['supplier_id'].">".$suppliers[$i]['supplier_name']."</option>"; } ?>
	</select>
	</td>
	</tr>
	<tr  bgcolor="#FFFFFF">
