...
<?php  session_start(); if(!isset($_SESSION['user'])){ echo "<script>alert('请先登录');</script>"; echo "<script>location.href='/zfApp/admin/index.php';</script>"; }else{ if($hs==0){ echo "暂无商品"; }else{ ?>
...
<tr>
	...
	<?php
 if($value['groupBuying']==null){ echo '无团购价'; }else{ $arr = explode('/',$value['groupBuying']); echo "该商品团购<strong>$arr[0]</strong>件以上有<strong>$arr[1]</strong>折优惠"; } ?></div></td>
	<td width="201"  bgcolor="#FFFFFF"><div>
	<?php  echo ($value['HolidayPreferences']==null)?无折扣:$value['HolidayPreferences']."折"; ?></div></td>
