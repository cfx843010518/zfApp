...
<?php
session_start(); if(!isset($_SESSION['user'])){ echo "<script>alert('请先登录');</script>"; echo "<script>location.href='/zfApp/admin/index.php';</script>"; }else{ if($total==0){ echo "本站暂无用户注册!"; }else{ ?> 
...
<td height="25" bgcolor="#FFFFFF"><div align="left"><span title="/zfApp/admin/Public/images/user/<?php echo $value["user_photo"];?>">查看照片</span></div></td>
...
<?php
echo "本站共有&nbsp;"; echo $total; echo "&nbsp;条记录&nbsp;"; echo "每页显示&nbsp;"; echo $size; echo "&nbsp;条&nbsp;"; echo "第&nbsp;"; echo $page_id; echo "&nbsp;页/共&nbsp;"; echo $page_num; echo "&nbsp;页&nbsp;"; if($page_id>=1 && $page_num>1){ echo "<a href=findUser?page_id=1&searchText=$searchText>第一页</a>"; } if($page_id>1 && $page_num>1){ echo "<a href=findUser?searchText=$searchText&page_id=".($page_id-1).">上一页</a>"; } if($page_id>=1 && $page_num>$page_id){ echo "<a href=findUser?searchText=$searchText&page_id=".($page_id+1).">下一页</a>"; } if($page_id>=1 && $page_num>1){ echo "<a href=findUser?searchText=$searchText&page_id=".$page_num.">尾页</a>"; } ?>
</td>
</tr>
</table>
<?php }?></form>
